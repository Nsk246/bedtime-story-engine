...
router, planner, storyteller, judge, a simulated child listener, and a reviser —
that gets you one worth reading out loud.
...
python smoke_test.py                     # 38 tests, no API key needed
```
...
**One shared spec.** The style rules — sentence length, what counts as too
scary, how the last hundred words must slow down — go to the writer, the judge
*and* the reviser. A judge grading against a spec the writer never saw produces
criticism nobody can act on.
...
| | first run | last run |
|---|---|---|
| length vs target | 27% | ~60% |
| beat labels leaking into the prose | 6 of 8 | 0 of 8 |
| refrain used the right number of times | 0 of 8 | 8 of 8 |
| score spread | every story 4.90 | 3.60 – 4.90 |

Two of those are worth explaining, because the fix wasn't the obvious one.
...
| — | `smoke_test.py` | 38 tests against a stubbed model |
...
hiding the beat labels from the writer, both came from reading the stories rather
than from the code.
